...
- Psychosocial theories count only if they are bona fide theories of aging (e.g., disengagement, continuity) explaining aging processes in psychosocial terms.

# Edge cases:

- “Hallmarks of aging” reviews: are valid aging-related papers
- Senolytics/senescence: are valid aging-related papers if positioned as testing whether senescence is a causal driver of aging broadly.
- Epigenetic clocks: are valid aging-related papers when discussing whether clocks capture causal programs vs damage and testing those claims; not valid when used solely for prediction.
- Psychosocial theories (e.g., disengagement, continuity) are valid, if they contain valid aging theory.
- If paper discuss general aging and paper title contains words "theory" or similiar or if authors propose that they establish/review a biological theory, most probalby is valid.
- Concepts that propose a new organizing framework for understanding multiple processes of aging (e.g., 'metabolaging') with the intent to unify, explain, or predict aspects of the aging process are to be classified as valid.
- You are given abtracts, so infer what could be the full context when analyzing papers. 
...
Step 5: Other notes: review other given detailes/exlusion/edge cases criteria, consider their info for final verdict. You may be a bit flexible as there are no direclt rules on what are aging papers.
